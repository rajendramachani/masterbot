...
async function createJobBranch({ project, task, feat_name }) {
  ...
  // Write job.md onto the feat branch
  const jobContent = Buffer.from(task).toString('base64');
  let existingSha;
  try {
  ...
  const existingPRs = await githubApi(
    `${repo}/pulls?head=${encodeURIComponent(`${GH_OWNER}:${branch}`)}&base=${devBranch}&state=open`
  );
  ...
  if (existingPRs.length > 0) {
    ...
    pr = await githubApi(`${repo}/pulls`, {
      method: 'POST',
      body: JSON.stringify({
        title: `feat(${project}): ${feat_name || featId.slice(0, 8)}`,
        head: branch,
        base: devBranch,
        body: `## Task\n\n${task}\n\n**Project:** \`${project}\`\n**Feat ID:** \`${featId}\`\n\n_Generated by masterbot_`,
      }),
    });
    ...
    await githubApi(`${repo}/issues/${pr.number}/labels`, {
      method: 'POST',
      body: JSON.stringify({ labels: ['automated', 'masterbot', project] }),
    }).catch(err => console.warn(`[github] label error: ${err.message}`));
  }

  return { feat_id: featId, branch, project, dev_branch: devBranch, pr_number: pr.number, pr_url: pr.html_url };
}
